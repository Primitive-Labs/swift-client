...
| Concern | TS | Swift |
|---|---|---|
| Source of truth | `models.toml` | `models.toml` or `*schema.toml` (the plugin accepts both — one shared file drives both runtimes, #944) |
| Schema outside the client's tree | `js-bao-codegen-v2 generate -i ../models/models.toml -o src/models` — the barrel imports the schema relative to `-o`, so an out-of-tree `-i` works; `--schema-import <specifier>` overrides it for bundler aliases (#2889) | `bao-codegen.json` in the target's source directory names the input (#2889, below); the standalone CLI takes any `--input` |
| Banner-protected output | `// AUTO-GENERATED FROM models.toml — DO NOT EDIT.` | `// Generated by swift-bao-codegen — DO NOT EDIT.` |
| Unknown-key handling | strict by default; `{ strict: false }` to allow | strict by default; `--no-strict` to allow (rejects typo'd keys at codegen, mirroring `tomlLoader`) |
| Registration barrel self-check | `index.ts` re-loads the TOML on import and throws if the generated set drifts | `GeneratedModels.register(on:)` `precondition`s the registered model set equals the TOML set baked in at codegen time |
...
**`enum` and `auto_stamp` fields.** An `enum = [...]` on a string field
emits a nested `<Field>Value` enum (advisory typed view; the wire stays a
plain string) — the type name is sanitized so a keyword/leading-digit
field name still produces a legal identifier (`default` → `DefaultValue`).
`auto_stamp = "create" | "update" | "both"` rides inside the field's
`FieldDescriptor` literal, and the shared write path (`DynamicModel.save`)
stamps `Date.now()`-style epoch millis on insert/update — matching JS
`save()` (`create` on insert only, `update`/`both` on every write,
caller-set values win). `save()` also short-circuits when nothing changed,
mirroring JS's dirty-check (an `update`/`both` stamp counts as a change).
...
The plugin auto-detects any file whose name ends in `schema.toml`, plus
the JS-canonical `models.toml` (#944), and runs codegen on each.
Generated `.swift` files land in the plugin's
...
#### Pointing the plugin at a schema outside the target

One Primitive app with two clients — say a web client and a Swift client
in sibling directories — needs exactly **one** `models.toml`: its keys are
wire field names, so a second copy that drifts orphans the other client's
records. Since the plugin's scan only sees the target's own source files,
name the shared schema in a `bao-codegen.json` at the root of the target's
source directory (#2889):

```
repo/
├── models/
│   └── models.toml        ← the one schema, shared with the web client
├── web/                   ← js-bao-codegen-v2 -i ../models/models.toml -o src/models
└── ios/Sources/MyApp/
    └── bao-codegen.json
```
...
Three levels up: `MyApp` → `Sources` → `ios` → `repo`, then into `models/`.

- The path resolves against the target's source directory; absolute paths
  are used as-is.
- When the file is present it **replaces** the filename scan — the
  `*schema.toml` / `models.toml` search is skipped entirely.
- **Exactly one `input`.** All generated files for a target land in one
  `GeneratedModels` directory, where the codegen tool writes a single
  barrel and sweeps generated files it did not emit on that run, so a
  second schema would delete the first one's output. An `inputs` array is
  rejected rather than half-honored.
- Anything wrong — unreadable or malformed JSON, a missing/empty/non-string
  `input`, or a path that doesn't resolve to a file — **fails the build**
  naming the config file and the offending value. There is no silent
  fallback to the scan.
- Add `exclude: ["bao-codegen.json"]` to the target so SwiftPM doesn't warn
  about an unhandled file. The plugin reads the config off disk, not
  through `target.sourceFiles`, so excluding it changes nothing.
- Edits to the schema *itself* always rebuild — the resolved file is declared
  as an input of the codegen command, wherever it lives. But SwiftPM only
  re-runs plugins when it re-plans the build, and editing `bao-codegen.json`
  does not trigger one: after re-pointing `input` at a *different* schema, the
  next `swift build` keeps generating from the old one and succeeds. Force the
  re-plan with `touch Package.swift` (adding or removing a source file, or
  `swift package clean`, does it too). Creating or deleting the config file is
  itself a re-plan, so this only bites on an edit in place.

> **The plugin's other defaults are not configurable.** It invokes the
...
1. **Run codegen from a target pre-build phase**, so every entry point to the
   Xcode build — the Run button, `xcodebuild`, CI — regenerates. Declaring the
   schema as the phase's input file and a stamp as its output keeps it
   dependency-analysed, so an unchanged schema costs nothing (#2886). In
   xcodegen's `project.yml`:

    ```yaml
    preBuildScripts:
      - name: Generate models from schema.toml
        basedOnDependencyAnalysis: true
        inputFiles:  [$(SRCROOT)/Sources/MyApp/Models/schema.toml]
        outputFiles: [$(DERIVED_FILE_DIR)/models-codegen.stamp]
        script: |
          set -euo pipefail
          cd "$SRCROOT"
          # Xcode exports the target's SDK into every phase; the codegen tool
          # builds for the host.
          unset SDKROOT PLATFORM_NAME MACOSX_DEPLOYMENT_TARGET
          swift run --package-path . swift-bao-codegen \
            --input  Sources/MyApp/Models/schema.toml \
            --output Sources/MyApp/Models/Generated
          mkdir -p "$DERIVED_FILE_DIR"; touch "$DERIVED_FILE_DIR/models-codegen.stamp"
    ```

    Keep the same invocation in your build script (e.g. `run-ios.sh`) too, before
    `xcodegen generate` — that is what puts a NEWLY added model's file on disk in
    time for xcodegen to list it. A build-phase-only setup regenerates changed
    models fine, but cannot add a file to the project it is building.
...
**Net effect**: `swift test` works (SPM plugin path), every Xcode build works
(the pre-build phase), and there's no path that builds the codegen-emitted
types twice. The price of admission is the dual-step setup; every piece above
is covered in the Swift template's
[`project.yml`](../../templates/primitive-swift-template/project.yml)
(the pre-build phase),
[`scripts/generate-models.sh`](../../templates/primitive-swift-template/scripts/generate-models.sh)
(what it runs, plus the added-model check),
