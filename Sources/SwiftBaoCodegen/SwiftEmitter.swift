...
struct SwiftEmitter {
    ...
    private func crossDocumentFacade(schema: ParsedSchema) -> String {
        ...
        out += "        let result = try JsBaoClient.requireDefault().upsertShared(Self.primitiveSchema, id: id, values: primitiveValues(), on: upsertOn, in: documentId)\n"
        ...
        out += "    }\n\n"
        out += "    /// Insert-or-update this record in `documentId`, matched by the\n"
        out += "    /// NAMED unique constraint (single-field or compound). Mirrors the\n"
        out += "    /// JS client's `Model.upsertByUnique(constraintName, lookupValue,\n"
        out += "    /// data, options)` — the lookup values come straight from this\n"
        out += "    /// record's fields (every constraint field must be set), and `mode`\n"
        out += "    /// maps JS's flags: `.mustExist` ⇔ `objectMustExist`,\n"
        out += "    /// `.mustNotExist` ⇔ `objectMustNotExist`, `.either` ⇔ default.\n"
        out += "    /// Throws if the doc isn't open, the constraint isn't declared on\n"
        out += "    /// this model, or a constraint field is unset on this record.\n"
        out += "    ///\n"
        out += "    /// Returns the RESOLVED record: on the merge path its `id` is the\n"
        ...
        out += "        let result = try JsBaoClient.requireDefault().upsertByUniqueShared(Self.primitiveSchema, id: id, values: primitiveValues(), constraint: constraint, mode: mode, in: documentId)\n"
        out += "        if let resolved = \(typeName)(record: result.record) { return resolved }\n"
        out += "        var copy = self\n"
        out += "        copy.id = result.record.id\n"
        out += "        return copy\n"
